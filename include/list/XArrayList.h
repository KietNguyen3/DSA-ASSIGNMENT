 ...
 */
 ...
 void XArrayList<T>::copyFrom(const XArrayList<T> &list)
 {
    ...
    this->data = new T[capacity];
    for(int i = 0; i < this->count; i++)    this->data[i] = list.data[i];
 ...
 {
     int n = this->indexOf(item);
     if(n == -1) return false;
     if(removeItemData) removeItemData(data[n]);
     this->removeAt(n);
     return true;
 }
 ...
 int XArrayList<T>::indexOf(T item)
 {
     ...
     for(int i = 0; i < this->count; i++){
         if(this->itemEqual? this->itemEqual(this->data[i], item): this->data[i] == item){
             return i;
         }
     }
     return -1;
 ...
 void XArrayList<T>::ensureCapacity(int index)
 {
     if(this->count == this->capacity){
         int newCapacity = this->capacity * 1.5 + 1;
