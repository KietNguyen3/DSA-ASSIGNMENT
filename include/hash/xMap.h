    ...
    public:
        Entry(K key, V value)
        {
            if constexpr (std::is_pointer<K>::value){
                this->key = key ? new std::remove_pointer_t<K>(*key) : nullptr;
            }
            else{
                this->key = key;
            }
            if constexpr (std::is_pointer<V>::value){
                this->value = value ? new std::remove_pointer_t<V>(*value) : nullptr;
            }
            else{
                this->value = value;
            }
...
xMap<K, V>::xMap(const xMap<K, V> &map) {
    // Initialize members that don't depend on the input map
    ...
    // Do not copy deleteKeys and deleteValues, as per copyMapFrom's comment
    this->deleteKeys = nullptr;
    this->deleteValues = nullptr;
...
{
    // YOUR CODE IS HERE
    if(this != &map) this->copyMapFrom(map);
    return *this;
}
...
V xMap<K, V>::put(K key, V value)
{
    ...
    typename DLinkedList<Entry*>::Iterator it(&table[index], true);
    typename DLinkedList<Entry*>::Iterator end(&table[index], false);
    ...
    //Not Found
        DLinkedList<Entry*> *holder = &this->table[index];
...
V xMap<K, V>::remove(K key, void (*deleteKeyInMap)(K))
{
    ...
    DLinkedList<Entry*>* holder = &table[index];
    
    while(it != end){
        Entry* entry = *it;
        if(keyEQ(entry->key, key) || entry->key == key){
            V temp = entry->value;
            if(deleteKeyInMap) deleteKeyInMap(entry->key);
            holder->removeItem(entry, deleteEntry);
            this->count--;
            return temp;
        }
        it++;
...
bool xMap<K, V>::remove(K key, V value, void (*deleteKeyInMap)(K), void (*deleteValueInMap)(V))
{
    ...
    typename DLinkedList<Entry*>::Iterator it(holder, true);
    typename DLinkedList<Entry*>::Iterator end(holder, false);

    while(it!=end){
        Entry* entry = *it;
        if((entry->key == key && entry->value == value) || keyEQ(entry->key, key) && valueEQ(entry->value, value)){
            if(deleteKeyInMap) deleteKeyInMap(key);
            if(deleteValueInMap) deleteValueInMap(value);
            holder->removeItem(entry, deleteEntry);
            this->count--;
            return true;
        }
...
bool xMap<K, V>::containsValue(V value)
{
    ...
    for(int i = 0; i < this->capacity; i++){
        DLinkedList<Entry*> *holder = &table[i];
...
{
    // YOUR CODE IS HERE
    DLinkedList<K> keyHolder;

    for(int i = 0; i < this->capacity; i++){
        DLinkedList<Entry*>* holder = &this->table[i];
        if(holder->size() == 0) continue;
        typename DLinkedList<Entry*>::Iterator it(holder, true);
        typename DLinkedList<Entry*>::Iterator end(holder, false);
        while(it != end){
            Entry* entry = *it;
            keyHolder.add(entry->key);
            it++;
        }
    }
    return keyHolder;
...
{
    // YOUR CODE IS HERE
    DLinkedList<int> clashesHolder;
    for(int i = 0; i < this->capacity; i++){
        DLinkedList<Entry*>* holder = &this->table[i];
        clashesHolder.add(holder->size());
    }
    return clashesHolder;
}
